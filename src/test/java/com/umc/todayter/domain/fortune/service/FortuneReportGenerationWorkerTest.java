...
class FortuneReportGenerationWorkerTest {
    ...
    private final SazuManseClient sazuClient = mock(SazuManseClient.class);
    ...
    private final FortuneReportGenerationWorker worker = new FortuneReportGenerationWorker(
            progressService, sazuClient, promptProvider, openAiClient
    );
    ...
    void generateAdvancesAllStages() {
        ...
        when(sazuClient.calculate(context)).thenReturn(manseData);
        when(promptProvider.create(context, manseData)).thenReturn("prompt");
        when(openAiClient.generate("prompt")).thenReturn("report");

        worker.generate(1L);

        InOrder order = inOrder(progressService, sazuClient, promptProvider, openAiClient);
        order.verify(progressService).start(1L);
        order.verify(sazuClient).calculate(context);
    ...
    void generationFailureIsPersisted() {
        FortuneReportGenerationContext context = mock(FortuneReportGenerationContext.class);
        when(progressService.start(1L)).thenReturn(context);
        when(sazuClient.calculate(context)).thenThrow(new FortuneReportGenerationException(
                "SAZU_API_FAILED", "만세력 정보를 생성하지 못했습니다."
        ));

        worker.generate(1L);

        verify(progressService).fail(1L, "SAZU_API_FAILED", "만세력 정보를 생성하지 못했습니다.");
